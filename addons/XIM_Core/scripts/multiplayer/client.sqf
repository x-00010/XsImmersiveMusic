...
XIM_fncObtainGroupUnits = // defines XIM_fncObtainGroupUnits, which returns an array with all the units in the client's group, minus the client themselves
{
	...
	private _aSelfGroupUnits = units _gSelfGroup - [XIM_uSelf]; // gets all of the units within the _gSelfGroup group and puts them in an array, minus the client so the others may sync
...
{
	params [["_uEnemy", objNull]]; // defines the optional parameter, XIM_uEnemy
	if (!isNull _uEnemy) then // if there is an argument in position zero
	{
		XIM_uEnemy = _this select 0; // assign the first argument to XIM_uEnemy, which is most likely the same as _uInstigator from before
	}
...
// ======================================== MAIN ========================================
while {leader group XIM_uSelf == XIM_uSelf} do  // while the client is the leader of their group
{
