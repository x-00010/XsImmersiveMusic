...
fncXIM_Shuffler = {
	...
	if (XIM_bMusicDelayEnabled) then // if the music delay is enabled
	{
		...
		[{params["_trackname","_oGroupLeader"]; if (_bXIMCombatState == _oGroupLeader getVariable ["XIM_bCombat", false]) then {[_trackname] remoteExecCall ["playMusic", XIM_groupOwnerIDs, false]; [10,1] remoteExecCall ["fadeMusic",XIM_groupOwnerIDs,false];};},[_trackname, _oGroupLeader], _iRandomDelay] call CBA_fnc_waitAndExecute;
	}
